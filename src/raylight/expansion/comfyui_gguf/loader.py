...
def gguf_sd_loader(path, handle_prefix="model.diffusion_model.", return_arch=False, is_text_model=False, use_mmap=True):
    """
...
def gguf_tekken_tokenizer_loader(path, temb_shape):
    ...
    for idx, (token, toktype) in enumerate(zip(tokens, toktypes)):
        if toktype == 3:
            data["special_tokens"].append({"rank": idx, "token_str": token, "is_control": True})
        else:
