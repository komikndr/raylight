# Extremely modified from comfy-kitchen-distributed, until upstream merges MXFP8 DTensor/FSDP ops.
...
def install_mxfp8_patches() -> None:
    global _PATCHED
    ...
    from comfy_kitchen.tensor.mxfp8 import TensorCoreMXFP8Layout as KitchenTensorCoreMXFP8Layout

    try:
        from comfy.quant_ops import get_layout_class as comfy_get_layout_class

        TensorCoreMXFP8Layout = comfy_get_layout_class("TensorCoreMXFP8Layout") or KitchenTensorCoreMXFP8Layout
    except Exception:  # pragma: no cover
        TensorCoreMXFP8Layout = KitchenTensorCoreMXFP8Layout
    _PATCHED_LAYOUT = TensorCoreMXFP8Layout

    def maybe_register(op):
        def deco(fn):
            if op is not None:
                register_layout_op(op, TensorCoreMXFP8Layout)(fn)
            return fn
    ...
    def _scale_unblocked_shape(qtensor) -> tuple[int, int]:
        storage_shape = tuple(qtensor._qdata.shape)
        logical_cols = TensorCoreMXFP8Layout.get_logical_shape_from_storage(storage_shape)[1]
        return storage_shape[0], logical_cols // 32

    def _unblock_scale(qtensor, scale: torch.Tensor | None = None) -> torch.Tensor:
        scale = qtensor._params.scale if scale is None else scale
        num_rows, num_cols = _scale_unblocked_shape(qtensor)
        return from_blocked(scale.view(torch.uint8), num_rows=num_rows, num_cols=num_cols)
    ...
    def _wrap_mxfp8_tensor(qtensor, qdata: torch.Tensor, *, scale: torch.Tensor | None = None, orig_shape=None, transposed=None):
    ...
    def _normalize_slice_args(size: int, start, end, step) -> tuple[int, int, int]:
        ...
        if step != 1:
            raise NotImplementedError("MXFP8 only supports slice step=1")
    ...
    def _slice_rows_mxfp8(input_tensor, start, end):
        ...
        scale_rows = _unblock_scale(input_tensor)
        sliced_scale = scale_rows[start_storage:end_storage]
        reblocked_scale = _reblock_scale(sliced_scale, input_tensor._params.scale.dtype)
        new_orig_shape = (end - start, int(input_tensor._params.orig_shape[1]))
        return _wrap_mxfp8_tensor(input_tensor, sliced_qdata, scale=reblocked_scale, orig_shape=new_orig_shape)
    ...
    def post_all_gather(qtensor: QuantizedTensor, all_gather_outputs, metadata: Any, param_dtype: torch.dtype, *, out=None):
        ...
        if out is not None:
            ...
            out._params = params
            return None
        return QuantizedTensor(gathered_qdata, qtensor._layout_cls, params), (gathered_qdata, gathered_scale)

    if _ORIG_LAYOUT_PRE is _MISSING:
        _ORIG_LAYOUT_PRE = getattr(TensorCoreMXFP8Layout, "pre_all_gather", _MISSING)
    if _ORIG_LAYOUT_POST is _MISSING:
        _ORIG_LAYOUT_POST = getattr(TensorCoreMXFP8Layout, "post_all_gather", _MISSING)
    setattr(TensorCoreMXFP8Layout, "pre_all_gather", pre_all_gather)
    setattr(TensorCoreMXFP8Layout, "post_all_gather", post_all_gather)
    ...
    def _handle_all_gather_impl(op_all_gather, args, kwargs):
        ...
        q_args = list(args)
        q_args[input_idx] = input_tensor._qdata.contiguous().view(torch.uint8)
        q_ret = op_all_gather(*q_args, **kwargs)
        q_result, q_work = _extract_collective_result(q_ret)
        q_bytes = q_result if isinstance(q_result, torch.Tensor) else q_args[input_idx]
    ...
    def _handle_wait_tensor_impl(op_wait_tensor, args, kwargs):
        ...
        waited_s_bytes = op_wait_tensor(input_tensor._params.scale.contiguous().view(torch.uint8), *args[1:], **kwargs)
        return _wrap_mxfp8_tensor(
            input_tensor,
            waited_q_bytes.view(input_tensor._qdata.dtype),
            scale=waited_s_bytes.view(input_tensor._params.scale.dtype),
        )
    ...
    def handle_scatter(qt, args, kwargs):
        ...
        new_s_outputs = list(output_tensors)
        for idx, tensor in enumerate(output_tensors):
            if isinstance(tensor, QuantizedTensor):
                ...
                new_s_outputs[idx] = tensor._params.scale.contiguous().view(torch.uint8)
            ...
            if isinstance(entry, (list, tuple)):
                ...
                for tensor in entry:
                    if isinstance(tensor, QuantizedTensor):
                        ...
                        s_entry.append(tensor._params.scale.contiguous().view(torch.uint8))
                    else:
                        q_entry.append(tensor)
                        s_entry.append(tensor)
                q_inputs.append(q_entry)
                s_inputs.append(s_entry)
            else:
                q_inputs.append(entry)
                s_inputs.append(entry)
        ...
        s_result = op_scatter(new_s_outputs, s_inputs, *args[2:], **kwargs)
        q_list, q_work = _extract_collective_result(q_result)
        s_list, s_work = _extract_collective_result(s_result)
    ...
    def handle_alias(qt, args, kwargs):
        ...
        return _wrap_mxfp8_tensor(
            input_tensor,
            op_alias(input_tensor._qdata),
            scale=op_alias(input_tensor._params.scale),
    ...
    def _can_alias_as_logical_shape(input_tensor, requested_size: tuple[int, ...], requested_stride: tuple[int, ...], storage_offset: int) -> bool:
        ...
        if len(requested_size) != 2 or input_tensor._qdata.dim() != 2:
            return False
        return tuple(input_tensor._qdata.shape) == TensorCoreMXFP8Layout.get_storage_shape(requested_size)
    ...
    def _requested_shape_from_args(args, kwargs) -> tuple[int, ...] | None:
        raw_shape = kwargs.get("shape", kwargs.get("size"))
        ...
        if raw_shape is None:
            return None
        requested = tuple(int(dim) for dim in raw_shape)
        ...
        if known == 0:
            return None
        numel = 1
        for dim in args[0]._params.orig_shape:
            numel *= int(dim)
        inferred = numel // known
        if inferred * known != numel:
            return None
        return tuple(inferred if dim == -1 else dim for dim in requested)

    def _handle_safe_logical_view(op, args, kwargs):
        input_tensor = args[0]
        if not isinstance(input_tensor, QuantizedTensor):
            return op(*args, **kwargs)
        requested_size = _requested_shape_from_args(args, kwargs)
        if requested_size is None:
            return op(*[_dequantize_arg(arg) for arg in args], **kwargs)
        orig_shape = tuple(int(dim) for dim in input_tensor._params.orig_shape)
        if requested_size == orig_shape:
            return _alias_with_orig_shape(input_tensor, orig_shape)
        requested_stride = _contiguous_stride(requested_size)
        if _can_alias_as_logical_shape(input_tensor, requested_size, requested_stride, 0):
            return _alias_with_orig_shape(input_tensor, requested_size)
        return op(*[_dequantize_arg(arg) for arg in args], **kwargs)

    @maybe_register(op_view)
    def handle_view(qt, args, kwargs):
        return _handle_safe_logical_view(op_view, args, kwargs)

    @maybe_register(op_reshape)
    def handle_reshape(qt, args, kwargs):
        return _handle_safe_logical_view(op_reshape, args, kwargs)
    ...
    def handle_slice(qt, args, kwargs):
        ...
        if dim == 0:
            ...
            return _slice_rows_mxfp8(input_tensor, start, end)
        if dim == 1:
    ...
    def handle_split(qt, args, kwargs):
        ...
        return tuple(_slice_rows_mxfp8(input_tensor, start, min(start + split_size, logical_rows)) for start in range(0, logical_rows, split_size))
    ...
    def handle_split_with_sizes(qt, args, kwargs):
        ...
        for split_size in split_sizes:
            ...
            chunks.append(_slice_rows_mxfp8(input_tensor, start, end))
            start = end
        return tuple(chunks)
    ...
    def handle_cat(qt, args, kwargs):
        ...
        if any(tensor._params.orig_shape[1] != first._params.orig_shape[1] for tensor in tensors):
            return op_cat(*[_dequantize_arg(arg) for arg in args], **kwargs)
        qdata = op_cat([tensor._qdata for tensor in tensors], 0)
        scale = _reblock_scale(op_cat([_unblock_scale(tensor) for tensor in tensors], 0), first._params.scale.dtype)
        orig_rows = sum(int(tensor._params.orig_shape[0]) for tensor in tensors)
        return _wrap_mxfp8_tensor(first, qdata, scale=scale, orig_shape=(orig_rows, int(first._params.orig_shape[1])))
    ...
    def handle_new_zeros(qt, args, kwargs):
        ...
        raw_size = args[1] if len(args) > 1 else tuple(input_tensor._params.orig_shape)
        size = (raw_size,) if isinstance(raw_size, int) else tuple(raw_size)
        if len(size) != 2:
            return op_new_zeros(*[_dequantize_arg(arg) for arg in args], **kwargs)
        device = kwargs.get("device", input_tensor._qdata.device)
        storage_shape = TensorCoreMXFP8Layout.get_storage_shape(size)
        qdata = torch.zeros(storage_shape, device=device, dtype=input_tensor._qdata.dtype)
        scale_rows = torch.zeros((storage_shape[0], storage_shape[1] // 32), device=device, dtype=torch.uint8)
        scale = _reblock_scale(scale_rows, input_tensor._params.scale.dtype)
        return _wrap_mxfp8_tensor(input_tensor, qdata, scale=scale, orig_shape=size)

    _PATCHED = True


def restore_mxfp8_patches() -> None:
    global _PATCHED
    global _ORIG_LAYOUT_PRE, _ORIG_LAYOUT_POST, _PATCHED_LAYOUT, _ORIG_QT_PRE, _ORIG_QT_POST
    ...
    TensorCoreMXFP8Layout = _PATCHED_LAYOUT or KitchenTensorCoreMXFP8Layout

    if _ORIG_LAYOUT_PRE is _MISSING:
        if hasattr(TensorCoreMXFP8Layout, "pre_all_gather"):
            delattr(TensorCoreMXFP8Layout, "pre_all_gather")
    else:
        setattr(TensorCoreMXFP8Layout, "pre_all_gather", _ORIG_LAYOUT_PRE)

    if _ORIG_LAYOUT_POST is _MISSING:
        if hasattr(TensorCoreMXFP8Layout, "post_all_gather"):
            delattr(TensorCoreMXFP8Layout, "post_all_gather")
    else:
        setattr(TensorCoreMXFP8Layout, "post_all_gather", _ORIG_LAYOUT_POST)
