# GGUF quantization eager mode patches for Raylight
    ...
    if _ORIG_ALL_GATHER_INTO_TENSOR is None:
        ...
        def all_gather_into_tensor_patched(output_tensor, input_tensor, group=None, async_op=False):
            ...
            if not isinstance(output_tensor, QuantizedTensor) or not isinstance(input_tensor, QuantizedTensor):
                raise TypeError("GGUF eager all_gather_into_tensor requires QuantizedTensor input and output")
            ...
            if async_op:
                return _CompositeWork(q_work)
            output_tensor._params = TensorCoreGGUFLayout.Params(
                scale=input_tensor._params.scale,
                orig_dtype=input_tensor._params.orig_dtype,
                orig_shape=input_tensor._params.orig_shape,
                n_blocks_per_superblock=input_tensor._params.n_blocks_per_superblock,
                super_block_scale_scale=input_tensor._params.super_block_scale_scale,
                super_block_min_scale=input_tensor._params.super_block_min_scale,
                quantized_block_scale=input_tensor._params.quantized_block_scale,
                quantized_block_min=input_tensor._params.quantized_block_min,
            )
            return q_work
    ...
    if _ORIG_ALL_GATHER is None:
        ...
        def all_gather_patched(tensor_list, tensor, group=None, async_op=False):
            ...
            if not isinstance(tensor, QuantizedTensor) or not all(isinstance(t, QuantizedTensor) for t in tensor_list):
                raise TypeError("GGUF eager all_gather requires QuantizedTensor input and outputs")
            ...
            for out_tensor in tensor_list:
                out_tensor._params = TensorCoreGGUFLayout.Params(
                    scale=tensor._params.scale,
                    orig_dtype=tensor._params.orig_dtype,
                    orig_shape=tensor._params.orig_shape,
                    n_blocks_per_superblock=tensor._params.n_blocks_per_superblock,
                    super_block_scale_scale=tensor._params.super_block_scale_scale,
                    super_block_min_scale=tensor._params.super_block_min_scale,
                    quantized_block_scale=tensor._params.quantized_block_scale,
                    quantized_block_min=tensor._params.quantized_block_min,
                )
            return q_work
    ...
    if _ORIG_SCATTER is None:
        ...
        def scatter_patched(tensor, scatter_list=None, src=0, group=None, async_op=False):
            ...
            if not isinstance(tensor, QuantizedTensor) or not has_quant_inputs:
                raise TypeError("GGUF eager scatter requires QuantizedTensor output and scatter_list")
            ...
            if ref is not None:
                tensor._params = TensorCoreGGUFLayout.Params(
                    scale=ref._params.scale,
                    orig_dtype=ref._params.orig_dtype,
                    orig_shape=ref._params.orig_shape,
                    n_blocks_per_superblock=ref._params.n_blocks_per_superblock,
                    super_block_scale_scale=ref._params.super_block_scale_scale,
                    super_block_min_scale=ref._params.super_block_min_scale,
                    quantized_block_scale=ref._params.quantized_block_scale,
                    quantized_block_min=ref._params.quantized_block_min,
                )
            return q_work
