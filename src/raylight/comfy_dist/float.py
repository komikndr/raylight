...
def manual_stochastic_round_to_float8(x, dtype, generator=None, device_mesh=None):
    ...
    sign = sign * torch.where(normal_mask, (2.0 ** (exponent - EXPONENT_BIAS)) * (1.0 + abs_x), (2.0 ** (-EXPONENT_BIAS + 1)) * abs_x)
    ...
    if dtype == torch.float8_e4m3fn or dtype == torch.float8_e5m2:
        ...
        for i in range(0, value.shape[0], slice_size):
            output[i : i + slice_size].copy_(
                manual_stochastic_round_to_float8(value[i : i + slice_size], dtype, generator=generator, device_mesh=device_mesh)
            )
        return output
...
def stochastic_round_quantize_nvfp4_block(x, per_tensor_scale, generator):
    ...
    scaled_block_scales_fp8 = torch.clamp(
        ((torch.amax(torch.abs(x), dim=-1)) / F4_E2M1_MAX) / per_tensor_scale.to(x.dtype), max=F8_E4M3_MAX
