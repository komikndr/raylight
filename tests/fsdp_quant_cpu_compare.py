...
def _cuda_mem_mb() -> dict[str, float] | None:
    ...
    return {
        "alloc_mb": round(torch.cuda.memory_allocated() / (1024 * 1024), 2),
        "reserved_mb": round(torch.cuda.memory_reserved() / (1024 * 1024), 2),
        "max_alloc_mb": round(torch.cuda.max_memory_allocated() / (1024 * 1024), 2),
        "max_reserved_mb": round(torch.cuda.max_memory_reserved() / (1024 * 1024), 2),
    }


def _capture_mem(device_type: str) -> dict[str, Any]:
    snap: dict[str, Any] = {"rss_mb": round(rss_mb(), 2)}
    if device_type == "cuda":
        snap["cuda"] = _cuda_mem_mb()
    return snap
...
def _load_patch_fns(layout_key: str, device_type: str) -> PatchFns:
    cfg = LAYOUTS[layout_key]
    module = _load_module(
        f"raylight_patch_{layout_key}_{device_type}_compare",
        cfg["patch_paths"][device_type],
    )
    return PatchFns(install=getattr(module, cfg["install_name"]), restore=getattr(module, cfg["restore_name"]))



def _get_runtime_device(device_type: str) -> torch.device:
    if device_type == "cpu":
...
def run_case(layout_key: str, device_type: str, mode: str, target_gib: float, dim: int) -> dict[str, Any]:
    runtime_device = _get_runtime_device(device_type)
    if device_type == "cuda":
    ...
    report["mem_after_load"] = _capture_mem(device_type)
    ...
    if mode == "tiny":
        x = torch.ones((4, dim), dtype=torch.bfloat16, device=runtime_device)
        with torch.no_grad():
            y = built.model(x)
        report["mem_after_forward"] = _capture_mem(device_type)
        report["output_shape"] = list(y.shape)
        report["output_sum"] = float(y.float().sum().item())
    else:
        report["mem_after_forward"] = None
...
def main() -> None:
    ...
    patch_fns = _load_patch_fns(args.layout, args.device)
    dist.init_process_group(backend=_get_backend(args.device))
    patch_fns.install()
    try:
        result = run_case(layout_key=args.layout, device_type=args.device, mode=args.mode, target_gib=args.target_gib, dim=args.dim)
